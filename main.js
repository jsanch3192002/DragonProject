...
function updateCinematicMotion(t) {
  ...
  if (performance.now() > userInteractingUntil) {
    ...
    if (shotPhase < 1) {
      ...
      // Close face pass
      const closeBlend = smoothstep(2.0, 2.5, shotPhase) * (1 - smoothstep(2.65, 3.0, shotPhase));
      cameraTargetPosition.copy(dragon.position)
        .addScaledVector(flightDirection, distance * (0.28 - closeBlend * 0.18))
        .add(new THREE.Vector3(0.15, 0.18, 0.1));
    } else {
    ...
    cameraLookTarget.copy(dragon.position)
      .addScaledVector(flightDirection, 0.45)
